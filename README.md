...
3.  **Gitの個人情報設定**:
    ...
    ```
4. **iTerm2の設定**:
   iTerm2の配色や設定を反映させるには、手動でプロファイルをインポートする必要があります。
   iTerm2を開き、`Preferences` > `Profiles` タブに移動します。左下の `Other Actions...` をクリックし、`Import JSON Profiles` を選択してください。
   ファイル選択ダイアログで、このリポジトリ内の `iterm/com.googlecode.iterm2.plist` を選択します。
